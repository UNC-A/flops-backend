...
impl Data {
    ...
    /// Get one Message from Message ID.
    ...
    /// Only returns if Message is sent by the specified User.
    ...
    /// Get Messages based on a set of message IDs.
    ...
    /// Get all Messages sent in a Channel
    pub async fn message_get_many_channel(
        &self,
        id: impl Into<String>,
    ) -> crate::Result<Vec<Message>> {
        Ok(collect!(
            self.messages.find(doc!("channel": id.into()), None).await?
        ))
    }
    /// Get many Messages from many Channels using Channel IDs.
